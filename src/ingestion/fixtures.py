...
The application must work without any API credentials. These helpers load the
committed, synthetic fixture files from ``data/raw``.
"""
...
def load_sandbox_fixture(path: Path | None = None) -> Dict[str, Any]:
    """Load the TrueLayer sandbox example fixture."""
...
def load_synthetic_transactions(path: Path | None = None) -> List[Dict[str, Any]]:
    """Load the synthetic UK transaction CSV as a list of records."""
